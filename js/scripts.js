...
//        $("#result_estampa").html(parametros.estampa.replace("_", " "));
...
//        $("#result_embalagem").html(parametros.embalagem);
    ...
    function atualizar_localStorange(parametros){
    ...
    //verificar local storange e atualizar variável parametros pesquisa
    atualizar_localStorange(parametros_pesquisa);
...
function getDados(){
    ...
    if($('#branca').hasClass("selected")){
     ...
     if($('#gola_v').hasClass("selected")){
        ...
        dados.gola = 'gola_normal';
    }
    
  
    if($('#q150').hasClass("selected")){
        dados.qualidade = 'q150';
    }else{
        dados.qualidade = 'q190';
    }
  
    if($('#estampa').val()){
        dados.estampa =$('#estampa').val();
    }
     if($('#embalagem').val()){
        dados.embalagem =$('#embalagem').val();
    }
    if($('#quantidade').val()){
        dados.quantidade =$('#quantidade').val();
    }
    
    parametros_pesquisa.quantidade= dados.quantidade;
    parametros_pesquisa.cor= dados.cor;
    parametros_pesquisa.gola= dados.gola;
    parametros_pesquisa.qualidade = dados.qualidade;
    parametros_pesquisa.estampa= dados.estampa;
    parametros_pesquisa.embalagem= dados.embalagem;
   atualizar_localStorange(parametros_pesquisa);
    atualizar_orcamento(parametros_pesquisa);
